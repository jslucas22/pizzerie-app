...
public class EmployeeCheckPadEditValidation : IValidationStrategy<EmployeeCheckPadEditRequest>
{
    public ContentResponse Validate(EmployeeCheckPadEditRequest? model)
    {
        if (model == null)
        {
        ...
        if (model.Products != null && model.Products.Any(item => item.Quantity <= 0))
        {
            return new ContentResponse()
            {
                Success = false,
